...
const createTweetElement = tweetObj => {
  ...
  //has to contain HTML structure of the tweet
  const newTweet =
  `
  <article>
...
const renderTweets = arrayOfTweetObj => {
  //prepend (to see latest tweets first)
...
const resetErrorMessage = violation => {
  if (violation === 'over count') {
    ...
    $(".error-message").hide();
    $(".error-message").empty();
    $(".error-message").append("<p>Empty Tweet</p>");
    $(".error-message").slideDown("slow");
    $('textarea').focus();
  } else {
    $(".error-message").hide();
...
// re-size tweet-textbox automatically 
$(document).ready(function() {
  ...
  // hides new tweet form and allows drop down
  $('.new-tweet').hide();

  // New tweet drop down button
  ...
  $('.submit-and-display button').click(function(event) {
    ...
    if (dataLength > 140) {
      ...
      //Reset the character counter to 140 after submitting 
      $(this)
        .closest(".new-tweet")
        .find(".counter")
